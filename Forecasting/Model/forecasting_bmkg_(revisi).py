# -*- coding: utf-8 -*-
"""Forecasting BMKG (Revisi)

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16N4pm36aY_HXNCUgqUhVYF-N6QtD3YQe
...
# Perbaikan letak tanggal agar tidak NameError
...
# --- TRANSFORMASI TARGET KE LOG (Hanya untuk Data Latih) ---
# Kita tidak mengubah y_test ke log karena metrik evaluasi harus membandingkan mm asli
y_train_log = np.log1p(y_train)

"""##SVM"""

model_svm = SVR(kernel='rbf', C=5.0, epsilon=0.1)
model_svm.fit(X_train_scaled, y_train_log) # Train dengan Log

y_pred_svm_log = model_svm.predict(X_test_scaled)
y_pred_svm = np.expm1(y_pred_svm_log) # INVERSE TRANSFORM ke skala mm asli
y_pred_svm = np.clip(y_pred_svm, a_min=0, a_max=None)

mae_svm = mean_absolute_error(y_test, y_pred_svm)
rmse_svm = np.sqrt(mean_squared_error(y_test, y_pred_svm))

# Plotting (Hapus baris dates_test karena sudah didefinisikan di atas)
...
model_rf.fit(X_train_scaled, y_train_log) # Train dengan Log

y_pred_rf_log = model_rf.predict(X_test_scaled)
y_pred_rf = np.expm1(y_pred_rf_log) # INVERSE TRANSFORM
...
model_xgb.fit(X_train_scaled, y_train_log) # Train dengan Log

y_pred_xgb_log = model_xgb.predict(X_test_scaled)
y_pred_xgb = np.expm1(y_pred_xgb_log) # INVERSE TRANSFORM
...
model_lgb.fit(X_train_scaled, y_train_log) # Train dengan Log

y_pred_lgb_log = model_lgb.predict(X_test_scaled)
y_pred_lgb = np.expm1(y_pred_lgb_log) # INVERSE TRANSFORM
...
model_lstm.fit(X_train_3d, y_train_log, epochs=50, batch_size=16, verbose=0) # Train dengan Log

y_pred_lstm_log = model_lstm.predict(X_test_3d).flatten()
y_pred_lstm = np.expm1(y_pred_lstm_log) # INVERSE TRANSFORM
...
fitur_terlatih = list(scaler.feature_names_in_)
fitur_eksogen = fitur_terlatih

baseline_exogenous = X_test[fitur_eksogen].iloc[-1]
...
for i in range(max_forecast):
    ...
    current_row = pd.Series(index=fitur_terlatih, dtype='float64')
    for col in fitur_terlatih:
        if col in baseline_exogenous.index:
            current_row[col] = baseline_exogenous[col]
    features_to_scale = pd.DataFrame([current_row])[fitur_terlatih]
    features_scaled = scaler.transform(features_to_scale)
    features_3d = np.reshape(features_scaled, (1, 1, features_scaled.shape[1]))

    pred_val_log = model_lstm.predict(features_3d, verbose=0).flatten()[0]
